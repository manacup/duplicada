...
/**
 * Valida si l'entrada de les coordenades és vàlida segons la direcció.
 * @param {string} coordinates - Les coordenades introduïdes.
 * @param {string} direction - La direcció seleccionada ("horitzontal" o "vertical").
 * @returns {boolean} - True si les coordenades són vàlides, false altrament.
 */
function isValidCoordinatesInput(coordinates, direction) {
    ...
    const hasLetter = letterRegex.test(coordinates);
    const hasNumber = numberRegex.test(coordinates);
    ...
    if (direction === 'horitzontal') {
        return letterRegex.test(coordinates[0]);
    } else if (direction === 'vertical') {
        return numberRegex.test(coordinates[0]);
    }

    return false; // Si la direcció no és vàlida
}
/* ////////////////
